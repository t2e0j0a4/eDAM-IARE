...
const About = () => {
  ...
  return (
    <div>
    <h1 className=" pt-4 px-10  text-blue-900 hover:text-violet-700 font-semibold text-3xl break-words">About e-DAM</h1>
    <div className="line my-2 w-[6%] h-[2px] mx-10 mt-5 rounded-sm bg-violet-700"></div>
  <section className="aboutUs w-[100%]  flex flex-col items-center justify-center pt-4 pb-10">
    {members.map((member)=>{
      const {id,name,role,desc,linkedin} = member;
      return (
        <div className="eachMember flex items-center flex-col md:flex-row w-[100%] max-w-[90%] mx-auto justify-center my-5 py-5 px-10 rounded-md hover:bg-violet-200 transition-all" key={id}>
          ...
          <div className="flex flex-col items-center my-2">
            <p className="text-center break-words text-black text-md">{desc}</p>
          </div>
        </div>
      )
    })}
